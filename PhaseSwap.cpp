...
void spec(imageP inImage,imageP outImage,imageP outImage2, bool scale = true){
	...
	MagfromComplex(CP,outImage2,scale);
	PhafromComplex(CP,outImage,scale);

	
}


void PhaseSwap(imageP in1, imageP in2, imageP out1, imageP out2){
	
	int total = in1->width * in1->height;
	
	
	out1->width = in1->width;
	out1->height = in1->height;
	out2->width = in1->width;
	out2->height = in1->height;
	out1->image = (uchar *) malloc(total);
	out2->image = (uchar *) malloc(total);
	
	imageP mag1, mag2, pha1, pha2;
	...
	spec(in2,pha2,mag2,false);
	spec(in1,pha1,mag1,false);

	
	//combine pha2 + mag1 and store into pha2
	for(int i = 0; i < total; ++i){
		int cur = pha2->image[i] + mag1->image[i];
		if (cur>255) cur = 255;
		pha2->image[i] = cur;
	}

	//combine pha1 +mag2 and store into pha1
	for(int i = 0; i < total; ++i){
		int cur = pha1->image[i] + mag2->image[i];
		if (cur>255) cur = 255;
		pha1->image[i] = cur;
	}
	
	complexS P1 = CompfromImag(pha1);
	complexS P2 = CompfromImag(pha2);
	

	complexP s2 = TwoDFFT(&P1,1,in1->width,in1->height);
	complexP s1 = TwoDFFT(&P2,1,in1->width,in1->height);
	

	MagfromComplex(s1,out1);
	MagfromComplex(s2,out2);
